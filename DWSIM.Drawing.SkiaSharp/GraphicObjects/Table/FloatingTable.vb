                        ...
                        If MSObj.Flowsheet.FlowsheetOptions.DisplayFloatingTableCompoundAmounts And (Y + DeltaY) > 0 Then
                            ...
                            Select Case ABasis
                                Case CompositionBasis.Mass_Flows
                                    ...
                                    canvas.DrawText(bpval2, New SKPoint(X + maxL4 + (maxL5 - MeasureString(bpval2, tpaint).Width) + Padding + 3, Y + (n + 4) * maxH + Padding + DeltaY), tpaint)
                                    p = MSObj.GetPhase("Vapor")
                                    bpval2 = p.Properties.massfraction.GetValueOrDefault.ConvertFromSI(convertfrom).ToString(nf)
                                    canvas.DrawText(bpval2, New SKPoint(X + maxL4 + maxL5 + (maxL6 - MeasureString(bpval2, tpaint).Width) + Padding + 3, Y + (n + 3) * maxH + Padding + DeltaY), tpaint)
                                    bpval2 = p.Properties.massflow.GetValueOrDefault.ConvertFromSI(convertfrom).ToString(nf)
                                    canvas.DrawText(bpval2, New SKPoint(X + maxL4 + maxL5 + (maxL6 - MeasureString(bpval2, tpaint).Width) + Padding + 3, Y + (n + 4) * maxH + Padding + DeltaY), tpaint)
                                    p = MSObj.GetPhase("Liquid1")
                                    bpval2 = p.Properties.massfraction.GetValueOrDefault.ConvertFromSI(convertfrom).ToString(nf)
                                    canvas.DrawText(bpval2, New SKPoint(X + maxL4 + maxL5 + maxL6 + (maxL7 - MeasureString(bpval2, tpaint).Width) + Padding + 3, Y + (n + 3) * maxH + Padding + DeltaY), tpaint)
                                    bpval2 = p.Properties.massflow.GetValueOrDefault.ConvertFromSI(convertfrom).ToString(nf)
                                    canvas.DrawText(bpval2, New SKPoint(X + maxL4 + maxL5 + maxL6 + (maxL7 - MeasureString(bpval2, tpaint).Width) + Padding + 3, Y + (n + 4) * maxH + Padding + DeltaY), tpaint)
                                    p = MSObj.GetPhase("Liquid2")
                                    bpval2 = p.Properties.massfraction.GetValueOrDefault.ConvertFromSI(convertfrom).ToString(nf)
                                    canvas.DrawText(bpval2, New SKPoint(X + maxL4 + maxL5 + maxL6 + maxL7 + (maxL8 - MeasureString(bpval2, tpaint).Width) + Padding + 3, Y + (n + 3) * maxH + Padding + DeltaY), tpaint)
                                    bpval2 = p.Properties.massflow.GetValueOrDefault.ConvertFromSI(convertfrom).ToString(nf)
                                    canvas.DrawText(bpval2, New SKPoint(X + maxL4 + maxL5 + maxL6 + maxL7 + (maxL8 - MeasureString(bpval2, tpaint).Width) + Padding + 3, Y + (n + 4) * maxH + Padding + DeltaY), tpaint)
                                    p = MSObj.GetPhase("Solid")
                                    bpval2 = p.Properties.massfraction.GetValueOrDefault.ConvertFromSI(convertfrom).ToString(nf)
                                    canvas.DrawText(bpval2, New SKPoint(X + maxL4 + maxL5 + maxL6 + maxL7 + maxL8 + (maxL9 - MeasureString(bpval2, tpaint).Width) + Padding + 3, Y + (n + 3) * maxH + Padding + DeltaY), tpaint)
                                    bpval2 = p.Properties.massflow.GetValueOrDefault.ConvertFromSI(convertfrom).ToString(nf)
                                    canvas.DrawText(bpval2, New SKPoint(X + maxL4 + maxL5 + maxL6 + maxL7 + maxL8 + (maxL9 - MeasureString(bpval2, tpaint).Width) + Padding + 3, Y + (n + 4) * maxH + Padding + DeltaY), tpaint)
                                Case CompositionBasis.Mass_Fractions
                                    atext = MSObj.Flowsheet.GetTranslatedString("FraoMssica")
                                    p = MSObj.GetPhase("Vapor")
                                    bpval2 = p.Properties.massfraction.GetValueOrDefault.ConvertFromSI(convertfrom).ToString(nf)
