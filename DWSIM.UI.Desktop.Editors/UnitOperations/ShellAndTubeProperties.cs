...
namespace DWSIM.UI.Desktop.Editors
{
    public class ShellAndTubePropertiesView : DynamicLayout
    {
        ...
        void Init()
        {
            ...
            tbBaffleSpacing.Text = cv.ConvertFromSI(su.thickness, st.Shell_BaffleSpacing / 1000.0).ToString(nf);
            tbNumberOfShellPasses.Text = st.Shell_NumberOfPasses.ToString();
            tbNumberOfTubesPerShell.Text = st.Tube_NumberPerShell.ToString();
            tbShellDi.Text = cv.ConvertFromSI(su.diameter, st.Shell_Di / 1000.0).ToString(nf);
            ...
            tbTubeDe.Text = cv.ConvertFromSI(su.diameter, st.Tube_De / 1000.0).ToString(nf);
            tbTubeDi.Text = cv.ConvertFromSI(su.diameter, st.Tube_Di / 1000.0).ToString(nf);
            ...
            tbTubePitch.Text = cv.ConvertFromSI(su.thickness, st.Tube_Pitch / 1000.0).ToString(nf);
            tbTubeRoughness.Text = cv.ConvertFromSI(su.diameter, st.Tube_Roughness / 1000.0).ToString(nf);
            ...
            tbBaffleSpacing.TextChanged += (sender, e) => { if (((TextBox)sender).Text.IsValidDoubleExpression()) st.Shell_BaffleSpacing = cv.ConvertToSI(su.thickness, ((TextBox)sender).Text.ParseExpressionToDouble()) * 1000; };
            ...
            tbShellDi.TextChanged += (sender, e) => { if (((TextBox)sender).Text.IsValidDoubleExpression()) st.Shell_Di = cv.ConvertToSI(su.diameter, ((TextBox)sender).Text.ParseExpressionToDouble()) * 1000; };
            ...
            tbTubeDe.TextChanged += (sender, e) => { if (((TextBox)sender).Text.IsValidDoubleExpression()) st.Tube_De = cv.ConvertToSI(su.diameter, ((TextBox)sender).Text.ParseExpressionToDouble()) * 1000; };
            tbTubeDi.TextChanged += (sender, e) => { if (((TextBox)sender).Text.IsValidDoubleExpression()) st.Tube_Di = cv.ConvertToSI(su.diameter, ((TextBox)sender).Text.ParseExpressionToDouble()) * 1000; };
            ...
            tbTubePitch.TextChanged += (sender, e) => { if (((TextBox)sender).Text.IsValidDoubleExpression()) st.Tube_Pitch = cv.ConvertToSI(su.thickness, ((TextBox)sender).Text.ParseExpressionToDouble()) * 1000; };
            tbTubeRoughness.TextChanged += (sender, e) => { if (((TextBox)sender).Text.IsValidDoubleExpression()) st.Tube_Roughness = cv.ConvertToSI(su.diameter, ((TextBox)sender).Text.ParseExpressionToDouble()) * 1000; };
