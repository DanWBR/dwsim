...
Public Class CompositionEditorForm
    ...
    Public Componentes As Dictionary(Of String, ICompound)
    ...
    Public SU As Global.DWSIM.SharedClasses.SystemsOfUnits.Units
    Public NF As String = ""
    ...
    Private Sub CompositionEditorForm_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        For Each comp In Me.Componentes.Values
            ...
            GridComp.Rows(GridComp.Rows.Count - 1).HeaderCell.Value = DWSIM.App.GetLocalString(comp.Name) & " (" & comp.ConstantProperties.OriginalDB & ")"
            GridComp.Rows(GridComp.Rows.Count - 1).HeaderCell.Tag = comp.Name
            ComboBox1.Items.Add(DWSIM.App.GetLocalString(comp.Name))
        Next
        ...
        If Not Me.Label2.Text = DWSIM.App.GetLocalString("Erro") Then
            ...
            If Me.RadioButton1.Checked Then
                ...
                For Each row In Me.GridComp.Rows
                    Me.Componentes(row.HeaderCell.Tag).MoleFraction = row.Cells(0).Value
                Next
                For Each comp In Me.Componentes.Values
            ...
            ElseIf Me.RadioButton2.Checked Then
                ...
                Call Me.Button23_Click(sender, e)
                For Each row In Me.GridComp.Rows
                    Me.Componentes(row.HeaderCell.Tag).MassFraction = row.Cells(0).Value
                Next
                For Each comp In Me.Componentes.Values
            ...
            ElseIf Me.RadioButton3.Checked Then
                ...
                For Each row In Me.GridComp.Rows
                    Me.Componentes(row.HeaderCell.Tag).MoleFraction = row.Cells(0).Value / total
                Next
                For Each comp In Me.Componentes.Values
                    mtotal += comp.MoleFraction.GetValueOrDefault * comp.ConstantProperties.Molar_Weight
                Next
            ...
            ElseIf Me.RadioButton4.Checked Then
                ...
                For Each row In Me.GridComp.Rows
                    Me.Componentes(row.HeaderCell.Tag).MassFraction = row.Cells(0).Value / total
                Next
                For Each comp In Me.Componentes.Values
                    mmtotal += comp.MassFraction.GetValueOrDefault / comp.ConstantProperties.Molar_Weight
                Next
            ...
            ElseIf Me.RadioButton5.Checked Then
                ...
                For Each row In Me.GridComp.Rows
                    If DWSIM.App.GetLocalString(row.HeaderCell.Tag) = Me.ComboBox1.SelectedItem.ToString Then
                        Me.Componentes(row.HeaderCell.Tag).MoleFraction = row.Cells(0).Value / 1000 * liqdens(i) / Me.Componentes(row.HeaderCell.Tag).ConstantProperties.Molar_Weight * 1000 / total
                    Else
                        Me.Componentes(row.HeaderCell.Tag).MoleFraction = row.Cells(0).Value / total
                    End If
                    i += 1
            ...
            ElseIf Me.RadioButton6.Checked Then
                ...
                For Each row In Me.GridComp.Rows
                    If DWSIM.App.GetLocalString(row.HeaderCell.Tag) = Me.ComboBox1.SelectedItem.ToString Then
                        Me.Componentes(row.HeaderCell.Tag).MoleFraction = row.Cells(0).Value / Me.Componentes(row.HeaderCell.Tag).ConstantProperties.Molar_Weight * 1000 / total
                    Else
                        Me.Componentes(row.HeaderCell.Tag).MoleFraction = row.Cells(0).Value / total
                    End If
                Next
            ...
            ElseIf Me.RadioButton7.Checked Then
                ...
                i = 0
                For Each row In Me.GridComp.Rows
                    Me.Componentes(row.HeaderCell.Tag).MassFraction = row.Cells(0).Value * liqdens(i) / mtotal
                    i += 1
                Next
                mmtotal = 0.0#
                For Each comp In Me.Componentes.Values
    ...
    Private Sub RadioButton_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles RadioButton1.CheckedChanged,
        ...
        If Me.loaded Then
            ...
            If Not Me.Label2.Text = DWSIM.App.GetLocalString("Erro") Then
                ...
                If Me.RadioButton1.Checked Then
                    For Each row In Me.GridComp.Rows
                        row.Cells(0).Value = Me.Componentes(row.HeaderCell.Tag).MoleFraction
                    Next
                ElseIf Me.RadioButton2.Checked Then
                    For Each row In Me.GridComp.Rows
                        row.Cells(0).Value = Me.Componentes(row.HeaderCell.Tag).MassFraction
                    Next
                ElseIf Me.RadioButton3.Checked Then

                    For Each row In Me.GridComp.Rows
                        row.Cells(0).Value = Converter.ConvertFromSI(SU.molarflow, Me.Componentes(row.HeaderCell.Tag).MoleFraction.GetValueOrDefault * Q)
                    Next
                ElseIf Me.RadioButton4.Checked Then

                    For Each row In Me.GridComp.Rows
                        row.Cells(0).Value = Converter.ConvertFromSI(SU.massflow, Me.Componentes(row.HeaderCell.Tag).MassFraction.GetValueOrDefault * W)
                    Next
                ElseIf Me.RadioButton5.Checked Then
                    'molarity = mol solute per liter solution
                    Dim n As Integer = Me.Componentes.Count
                    ...
                    For Each row In Me.GridComp.Rows
                        If DWSIM.App.GetLocalString(row.HeaderCell.Tag) = Me.ComboBox1.SelectedItem.ToString Then
                            row.Cells(0).Value = Me.Componentes(row.HeaderCell.Tag).MoleFraction.GetValueOrDefault * Q * Me.Componentes(row.HeaderCell.Tag).ConstantProperties.Molar_Weight / 1000 / liqdens(i) * 1000
                        Else
                            row.Cells(0).Value = Me.Componentes(row.HeaderCell.Tag).MoleFraction.GetValueOrDefault * Q
                        End If
                        i += 1
                    Next
                ElseIf Me.RadioButton6.Checked Then
                    ...
                    For Each row In Me.GridComp.Rows
                        If DWSIM.App.GetLocalString(row.HeaderCell.Tag) = Me.ComboBox1.SelectedItem.ToString Then
                            row.Cells(0).Value = Me.Componentes(row.HeaderCell.Tag).MassFraction.GetValueOrDefault * W
                        Else
                            row.Cells(0).Value = Me.Componentes(row.HeaderCell.Tag).MoleFraction.GetValueOrDefault * Q
                        End If
                    Next
                ElseIf Me.RadioButton7.Checked Then
                    ...
                    i = 0
                    For Each row In Me.GridComp.Rows
                        row.Cells(0).Value = Me.Componentes(row.HeaderCell.Tag).MoleFraction * Me.Componentes(row.HeaderCell.Tag).ConstantProperties.Molar_Weight / liqdens(i) / totalvol
                        i += 1
                    Next
                    ipp = Nothing
                End If
