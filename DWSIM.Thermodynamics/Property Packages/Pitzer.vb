...
Namespace PropertyPackages
    ...
    <System.Runtime.InteropServices.Guid(PitzerPropertyPackage.ClassId)>
    <System.Serializable()> Public Class PitzerPropertyPackage

        Inherits PropertyPackages.ElectrolyteBasePropertyPackage

        Public Shadows Const ClassId As String = "3f42dd0d-54b9-4fb8-a07f-1ecfca39a988"

        Private m_props As New PropertyPackages.Auxiliary.PROPS
        Public m_pitzer As New PropertyPackages.Auxiliary.Pitzer
        ...
        Public Overrides Sub DisplayEditingForm()
            ...
            Else
                Dim f As New FormConfigPitzer() With {._form = Flowsheet, ._pp = Me, ._comps = Flowsheet.SelectedCompounds}
                f.ShowDialog()
            End If
...
#Region "    DWSIM Functions"
        ...
        Public Overrides Sub DW_CalcProp(ByVal [property] As String, ByVal phase As Phase)
            ...
            Select Case [property].ToLower
                ...
                Case "enthalpy", "enthalpynf"
                    ...
                    ElseIf phase = Phase.Vapor Then
                        result = Me.m_elec.LiquidEnthalpy(T, RET_VMOL(phase), constprops, Me.m_pitzer.GAMMA_MR(T + 0.1, RET_VMOL(phase), constprops), Me.m_pitzer.GAMMA_MR(T, RET_VMOL(phase), constprops), False)
                ...
                Case "osmoticcoefficient"
                    ...
                    For Each su As Interfaces.ICompound In Me.CurrentMaterialStream.Phases(0).Compounds.Values
                        constprops.Add(su.ConstantProperties)
                    Next
                    Me.CurrentMaterialStream.Phases(phaseID).Properties.osmoticCoefficient = Me.m_elec.OsmoticCoeff(RET_VMOL(phase), Me.m_pitzer.GAMMA_MR(T, RET_VMOL(phase), constprops), constprops)
                Case "freezingpoint"
                    Dim constprops As New List(Of Interfaces.ICompoundConstantProperties)
                    For Each su As Interfaces.ICompound In Me.CurrentMaterialStream.Phases(0).Compounds.Values
                        constprops.Add(su.ConstantProperties)
                    Next
                    Me.CurrentMaterialStream.Phases(phaseID).Properties.freezingPoint = Me.m_elec.FreezingPointDepression(RET_VMOL(phase), Me.m_pitzer.GAMMA_MR(T, RET_VMOL(phase), constprops), constprops)(0)
                Case "freezingpointdepression"
                    Dim constprops As New List(Of Interfaces.ICompoundConstantProperties)
                    For Each su As Interfaces.ICompound In Me.CurrentMaterialStream.Phases(0).Compounds.Values
                        constprops.Add(su.ConstantProperties)
                    Next
                    Me.CurrentMaterialStream.Phases(phaseID).Properties.freezingPointDepression = Me.m_elec.FreezingPointDepression(RET_VMOL(phase), Me.m_pitzer.GAMMA_MR(T, RET_VMOL(phase), constprops), constprops)(1)
                Case "ph"
                    Dim constprops As New List(Of Interfaces.ICompoundConstantProperties)
                    For Each su As Interfaces.ICompound In Me.CurrentMaterialStream.Phases(0).Compounds.Values
                        constprops.Add(su.ConstantProperties)
                    Next
                    Me.CurrentMaterialStream.Phases(phaseID).Properties.pH = Me.m_elec.pH(RET_VMOL(phase), T, Me.m_pitzer.GAMMA_MR(T, RET_VMOL(phase), constprops), constprops)
                Case Else
        ...
        Public Overrides Function DW_CalcEnthalpy(ByVal Vx As System.Array, ByVal T As Double, ByVal P As Double, ByVal st As State) As Double
            ...
            Select Case st
                Case State.Liquid
                    H = Me.RET_Hid(298.15, T, Vx) - Me.RET_HVAPM(Me.AUX_CONVERT_MOL_TO_MASS(Vx), T) - Me.m_elec.LiquidEnthalpy(T, Vx, constprops, Me.m_pitzer.GAMMA_MR(T + 0.1, Vx, constprops), Me.m_pitzer.GAMMA_MR(T, Vx, constprops), False)
                Case State.Solid
                    H = Me.RET_Hid(298.15, T, Vx) - Me.RET_HVAPM(Me.AUX_CONVERT_MOL_TO_MASS(Vx), T) - Me.m_elec.LiquidEnthalpy(T, Vx, constprops, Me.m_pitzer.GAMMA_MR(T + 0.1, Vx, constprops), Me.m_pitzer.GAMMA_MR(T, Vx, constprops), False) - Me.RET_HFUSM(Me.AUX_CONVERT_MOL_TO_MASS(Vx), T) - Me.m_elec.SolidEnthalpy(T, Vx, constprops)
                Case State.Vapor
        ...
        Public Overrides Function DW_CalcFugCoeff(ByVal Vx As System.Array, ByVal T As Double, ByVal P As Double, ByVal st As State) As Double()
            ...
            If st = State.Liquid Then
                ...
                ativ = Me.m_pitzer.GAMMA_MR(T, Vx, constprops)
